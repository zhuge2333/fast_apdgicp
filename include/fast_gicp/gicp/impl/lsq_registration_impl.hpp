...
void LsqRegistration<PointTarget, PointSource>::computeTransformation(PointCloudSource& output, const Matrix4& guess) {
  Eigen::Isometry3d x0_isom = Eigen::Isometry3d(guess.template cast<double>()); // guess matrix
  ...
  for (int i = 0; i < max_iterations_ && !converged_; i++) {
    ...
    Eigen::Isometry3d delta; // The relative transform
    if (!step_optimize(x0_isom, delta)) {
  ...
  final_transformation_ = x0_isom.cast<float>().matrix();
...
template <typename PointTarget, typename PointSource>
bool LsqRegistration<PointTarget, PointSource>::step_optimize(Eigen::Isometry3d& x0_isom, Eigen::Isometry3d& delta) {
  switch (lsq_optimizer_type_) {
    case LSQ_OPTIMIZER_TYPE::LevenbergMarquardt:
      return step_lm(x0_isom, delta);
    case LSQ_OPTIMIZER_TYPE::GaussNewton:
      return step_gn(x0_isom, delta);
  }
...
template <typename PointTarget, typename PointSource>
bool LsqRegistration<PointTarget, PointSource>::step_gn(Eigen::Isometry3d& x0_isom, Eigen::Isometry3d& delta) {
  Eigen::Matrix<double, 6, 6> H; // JTJ
  Eigen::Matrix<double, 6, 1> b; // JTr
  double y0 = linearize(x0_isom, &H, &b);

  Eigen::LDLT<Eigen::Matrix<double, 6, 6>> solver(H);
  Eigen::Matrix<double, 6, 1> d = solver.solve(-b);

  delta.setIdentity();
  delta.linear() = so3_exp(d.head<3>()).toRotationMatrix(); // so(3) To quaterniond, then to rotation matrix
  delta.translation() = d.tail<3>();

  x0_isom = delta * x0_isom; 
  final_hessian_ = H;
...
// lm优化
template <typename PointTarget, typename PointSource>
bool LsqRegistration<PointTarget, PointSource>::step_lm(Eigen::Isometry3d& x0_isom, Eigen::Isometry3d& delta) {
  ...
  for (int i = 0; i < lm_max_iterations_; i++) {
    ...
    Eigen::Isometry3d xi = delta * x0_isom;
    double yi = compute_error(xi);            // computed error
    double rho = (y0 - yi) / (d.dot(lm_lambda_ * d - b)); // Equa. (6.34) in the book
